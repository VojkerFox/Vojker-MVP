...
def process_symbol_logic(symbol_tensor): # Datan purku (M1, M5 ja RNAI-aggressio)
m1 = symbol_tensor[0]
m5 = symbol_tensor[1]
rnai = symbol_tensor[2, -1, 0]

    # --- 1. RAKENTEEN TUNNISTUS (M5 BOS) ---
    # Haetaan dynaamiset tasot ennen nykyistä kynttilää
    m5_res = jnp.max(m5[:-1, 1])
    m5_sup = jnp.min(m5[:-1, 2])

    # --- 2. LIGHTNING BOLT (Dynaaminen Entry) ---
    # Break: Nykyinen High/Low rikkoo M5-rakenteen
    break_long = m1[-1, 1] > m5_res
    break_short = m1[-1, 2] < m5_sup

    # Retest: Kynttilän häntä (wick) koskettaa murtunutta tasoa
    # Sallitaan 0.2 pipin liukuma (0.00002) dynaamiseen osumaan
    retest_long = m1[-1, 2] <= (m5_res + 0.00002)
    retest_short = m1[-1, 1] >= (m5_sup - 0.00002)

    # Trigger: Yhdistetään murtuma, retesti ja aito RNAI-voima
    # Kynnys 0.8 on säädetty Mikrobot-kuvan mukaiseen volyymiin
    is_long = break_long & retest_long & (rnai > 0.8)
    is_short = break_short & retest_short & (rnai < -0.8)

    # Signaalin koodaus: 0=IDLE, 1=LONG, 2=SHORT
    signal = jnp.where(is_long, 1, jnp.where(is_short, 2, 0))

    # --- 3. SALAMAN LAATIKKO (Positionointi & Hard SL) ---
    # Napataan 5 kynttilän LB-rakenne.
    # Lisätään 0.5 pointin (0.00005) puskuri estämään "uuvutus-exitit"
